 ...
 // Init and test if input file contains the right properties
  ...
  void Pset::read_ply(std::string path)
  {
     //FT xc = (FT)0., yc = (FT)0., zc = (FT)0.,
        std::cout<<"Reading PLY file: "<<path<<std::endl;
        Color c = {{ 0, 0, 0 }};
        ...
        if(has_normals)
          std::cout<<"PLY has normals."<<std::endl;
        if(has_alpha)
          std::cout<<"PLY has alpha channel"<<std::endl;
...
void Pset::correct_color(double val)
{
  for (size_t i=0; i < colors.size(); i++)
  {
    ...
    if (l < val)
    {
      ...
      unsigned char r = ( (unsigned char)(cval*colors[i][0]) <= 255) ? (unsigned char)(cval*colors[i][0]) : 255;
      unsigned char g = ( (unsigned char)(cval*colors[i][1]) <= 255) ? (unsigned char)(cval*colors[i][1]) : 255;
      unsigned char b = ( (unsigned char)(cval*colors[i][2]) <= 255) ? (unsigned char)(cval*colors[i][2]) : 255;
