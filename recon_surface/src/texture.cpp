...
{	
    ...
    ROS_DEBUG_STREAM("Number of faces of the mesh: " << nfaces);
    ROS_DEBUG_STREAM("Created a grid of " << l << " by " << l << " for per-face uv parametrization.");
    ROS_DEBUG_STREAM("Grid square size (pixels): " << t*texture_size);
	...
	for(int i=0; i< l;i++)
	{
		...
		if(fmod(percentage, 10.0f) < 0.1) {
            ROS_DEBUG("Baking texture... (%.2f%% done)", percentage);
        }
	}

    ROS_DEBUG_STREAM("Writing texture: " << output_path+"_texture.png");
