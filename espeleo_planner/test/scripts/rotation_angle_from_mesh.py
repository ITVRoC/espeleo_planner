...
def set_bullet_client(self, mesh_map, mesh_robot):
    pybullet_client.connect(p.GUI)

    pybullet_client.setGravity(0, 0, -10)

    map_start_pos = [0, 0, 0]
    map_start_orientation = pybullet_client.getQuaternionFromEuler([-math.pi / 2, 0, 0])
    map_id = pybullet_client.loadURDF(mesh_map, map_start_pos, map_start_orientation)

    robot_start_pos = [0, 0, 0]
    robot_start_orientation = pybullet_client.getQuaternionFromEuler([-math.pi / 2, 0, 0])
    robot_id = pybullet_client.loadURDF(self.robot_urdf, robot_start_pos, robot_start_orientation,
    ...
    pybullet_client.changeDynamics(self.robot_id, 0, lateralFriction=lf, spinningFriction=sf, rollingFriction=rf,
                                      linearDamping=ld, angularDamping=ad, contactStiffness=cf)
    pybullet_client.changeDynamics(self.robot_id, 1, lateralFriction=lf, spinningFriction=sf, rollingFriction=rf,
                                      linearDamping=ld, angularDamping=ad, contactStiffness=cf)
    pybullet_client.changeDynamics(self.robot_id, 2, lateralFriction=lf, spinningFriction=sf, rollingFriction=rf,
                                      linearDamping=ld, angularDamping=ad, contactStiffness=cf)
    pybullet_client.changeDynamics(self.robot_id, 3, lateralFriction=lf, spinningFriction=sf, rollingFriction=rf,
                                      linearDamping=ld, angularDamping=ad, contactStiffness=cf)
    pybullet_client.changeDynamics(self.robot_id, 4, lateralFriction=lf, spinningFriction=sf, rollingFriction=rf,
                                      linearDamping=ld, angularDamping=ad, contactStiffness=cf)
    pybullet_client.changeDynamics(self.robot_id, 5, lateralFriction=lf, spinningFriction=sf, rollingFriction=rf,
                                      linearDamping=ld, angularDamping=ad, contactStiffness=cf)
    pybullet_client.changeDynamics(self.map_id, -1, lateralFriction=lf, spinningFriction=sf, rollingFriction=rf,
