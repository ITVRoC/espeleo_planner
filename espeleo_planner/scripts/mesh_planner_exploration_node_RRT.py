...
class ExplorationRRTNode:
    ...
    def prepare_pointclouds(self, curr_lidar_pcloud, sampled_pc, src_point=None):
        ...
        downpcd_simple = pcd.voxel_down_sample(voxel_size=0.05)
        ...
        if src_point:
            ...
            camera = [src_point[0], src_point[1], src_point[2]]
            radius = diameter * 3
        ...
        np_points_filter = np_points[(degrees_u <= 25)]
        np_normals_filter = np_normals[(degrees_u <= 25)]

        bad_points = np_points[(degrees_u > 25)]

        print("Filtering points by distance..")
        # cdist was exploding the memory on the long run
        #filtering_by_dist = np.all(cdist(np_points_filter, bad_points) >= 0.60, axis=1)

        kd_tree = scipy.spatial.cKDTree(np_points_filter)
        filtering_by_dist = [True for _ in np_points_filter]
        for point in bad_points:
            near_idx = kd_tree.query_ball_point(point, r=0.60)
            for idx in near_idx:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            return_dict = rrt_planner.run(source_face_idx[0][0], target_faces, normalized_mi, is_debug=False)
            ...
            if self.graph_metric_type in return_dict:
                ...
                target_idx = return_dict[self.graph_metric_type]['target_idx']
                if target_idx != -1 and target_idx < len(self.mi_markers.markers):
                    del self.mi_markers.markers[target_idx]
            ...
            if selected_frontier_idx == -1:
                ...
                cooldown_secs = self.cooldown_time_secs * normalized_mi[selected_frontier_idx]
                self.cooldown_epoch = time.time() + cooldown_secs
                rospy.loginfo("\tRepeating planning for frontier: (%s) %s", selected_frontier_idx, selected_frontier)
                rospy.loginfo("\tCooling down for %s secs", cooldown_secs)
                self.rate_fast.sleep()

                # while time.time() <= self.cooldown_epoch:
                while not rospy.is_shutdown():
                    # forcing this specific frontier
                    rospy.loginfo("Local path search...")
                    #rospy.loginfo("\tCooling down time remaining: %s secs", self.cooldown_epoch - time.time())
                    ...
                    frontier_idx = mesh_helper.find_closer_centroid(filtered_points,
                                                                    target_positions[selected_frontier_idx],
                                                                    force_return_closer=True)

                    # if frontier_idx == -1:
                    #     rospy.logerr("No path found! get new target")
                    #     self.mplanner.action_client.cancel_all_goals()
                    #     break
