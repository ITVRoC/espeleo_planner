...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        try:
            ...
            # centroids = np.asarray(mesh.vertices)
            # normals = np.asarray(mesh.vertex_normals)

            #print("mesh.adjacency_list:", mesh.adjacency_list)

            # print("PUBLISH centroids[0]:", centroids[0], "centroids[-1]:", centroids[-1])
            ...
            centroids = mplanner_base.centroids
            normals = mplanner_base.normals
            ...
            # conn_nodes = nx.node_connected_component(G, source_face)
            # print("conn nodes:", conn_nodes)

            # len_connected = [len(c) for c in sorted(nx.connected_components(G), key=len, reverse=True)]
            # print("len_connected:", len_connected)
